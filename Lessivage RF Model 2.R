...
data$Mineralogy=as.factor(data$Mineralogy)
data$GeoUnit=as.factor(data$GeoUnit)
data$Deposit.Material=as.factor(data$Deposit.Material)
data$Drainage.Class=as.factor(data$Drainage.Class)
data$Depth.to.Gleying=as.factor(data$Depth.to.Gleying)
...
Zinputs=data[,c(4,11:38,51:58)]
str(Zinputs)

####Tune for depth of lessivage for ntree and mtry####
...
for(i in 1:50){
	...
	Z.rf=randomForest(Zlessivage~.,data=train,na.action=na.exclude,importance=T,ntree=800,mtry=8)
	zimp=cbind(zimp,unname(importance(Z.rf))[,1])
	oobRsqr=c(oobRsqr,Z.rf$rsq[800])
...
write.csv(zimp.summary,"FSSC_Zimportance3.csv", row.names=F)
...
for(i in 1:50){
	...
	for(j in 1:nrow(train)){test=subset(test,row.names(test)!=temp[j])}
	#run model 50 times with unique cal and val datasets
	D.rf=randomForest(I.E.Ratio~.,data=train,na.action=na.exclude,importance=T,ntree=375, mtry=20)
	Dimp=cbind(Dimp,unname(importance(D.rf))[,1])
	oobRsqr=c(oobRsqr,D.rf$rsq[375])
	#test model against valdation datasets
...
write.csv(Dimp.summary,"FSSC_Dimportance2.csv", row.names=F)
...
partial(D.rf, pred.var="I_Sand", plot=TRUE, rug = TRUE)
...
partial(D.rf, pred.var = "Drainage.Class", plot = TRUE, rug = TRUE)
